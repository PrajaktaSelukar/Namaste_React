...
#### Custom Hooks
 - Hooks are utility function like helper functions
 - Better to create Hooks inside utils folder
 - Prefer to create separate file for separate Hook
 - Recommended - Start Hook name with small cased **use** => This tells React that it's a Hook. Ex useRestaurantMenu

 - For simulating Offline connection, go to Network tab and change from No Throtling to Offline
 - For adding Emojis in VsCode, use command (Windows + .)
...
#### Optimizing Bundler
 ...
 - Implementing Lazy loading so that when we go to Grocery then only Grocery component is loaded. 
 - We will not load everything directly, we'll do lazy loading when required
```
const Grocery = lazy(() => {
    import("./components/Grocery")
});
```
 - This import is a function that will take the path of the Grocery
 - We have now splitted code into 2 JavaScript bundle
 - Suspence is a component which wraps the Grocery component
